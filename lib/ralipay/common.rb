module Ralipay::Common
  ...
  def self.build_sign data_array
    ...
    #签名
    if Ralipay.global_configs[:secure_type] == 'RSA'
      return self.rsa_sign(for_sign_string)
    elsif Ralipay.global_configs[:secure_type] == 'MD5'
  ...
  def self.rsa_sign for_sign_string
    #读取私钥文件
    rsa_private_key_file = File.read(Ralipay.global_configs[:rsa_private_key_path])
    #转换为openssl密钥
  ...
  def self.verify? for_sign_string, signed_string
    #读取公钥文件
    rsa_public_key_file = File.read(Ralipay.global_configs[:rsa_public_key_path])
    #转换为RSA对象
